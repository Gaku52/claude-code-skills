...
プロセスは独立したアドレス空間を持つ実行単位であり、スレッドはプロセス内でアドレス空間を共有する実行単位である。プロセスの生成（fork）はアドレス空間のコピーを伴うためコストが高いが、メモリ保護が強い。スレッドの生成はアドレス空間を共有するため高速だが、あるスレッドのバグ（不正なメモリアクセス等）がプロセス全体をクラッシュさせる。Webサーバーのnginxはマルチプロセスモデル、Node.jsは単一プロセス内のイベントループモデル、JavaのSpring Bootはマルチスレッドモデルを採用しており、用途に応じて選択される。詳細は次章 [[01-threads.md]] で扱う。
...
- [[01-threads.md]] -- スレッドと並行性: プロセス内の軽量実行単位
- [[02-synchronization.md]] -- 同期機構: ミューテックス、セマフォ、デッドロック
- [[03-scheduling.md]] -- スケジューリング詳細: CFS、リアルタイムスケジューリング
