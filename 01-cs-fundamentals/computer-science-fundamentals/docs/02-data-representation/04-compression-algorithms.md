...
- 2 進数の基礎 → 参照: [[00-binary-and-number-systems.md]]
...
```
  ...
  ステップ2: 頻度の低いものから順にペアにして結合（優先度付きキュー使用）
    ...
    反復1: C(1) と D(1) を結合 → [CD](2)
           残り: B:2  [CD]:2  R:2  A:5

    反復2: B(2) と [CD](2) を結合 → [B,CD](4)
           残り: R:2  [B,CD]:4  A:5

    反復3: R(2) と [B,CD](4) を結合 → [R,B,CD](6)
           残り: A:5  [R,B,CD]:6

    反復4: A(5) と [R,B,CD](6) を結合 → [A,R,B,CD](11)
           根ノード完成
...
→ [[05-storage-capacity.md]] -- データ量の感覚とストレージの基礎
