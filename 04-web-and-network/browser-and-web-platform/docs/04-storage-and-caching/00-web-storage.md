...
### Q1: localStorage と sessionStorage の容量制限はどのように決まるのか？

仕様（HTML Living Standard）では具体的な容量の数値を規定していない。
各ブラウザの実装が独自に上限を設定している。

| ブラウザ | localStorage | sessionStorage | 備考 |
|---------|-------------|----------------|------|
| Chrome | 約 5MB | 約 5MB | オリジンあたり |
| Firefox | 約 5MB | 約 5MB | オリジンあたり（設定で変更可能） |
| Safari | 約 5MB | 無制限※ | ※タブ閉じたら消えるため実質制限なし |
| Edge | 約 5MB | 約 5MB | Chrome と同等 |

容量の「5MB」は文字列ベースの換算であり、UTF-16 エンコーディングのため、
ASCII 文字だけなら約 250万文字、日本語（マルチバイト）でも同数の文字を保存できる。
ただし、JSON.stringify のオーバーヘッド（引用符、括弧など）も容量に含まれる点に注意。

### Q2: IndexedDB のパフォーマンスはどの程度か？

IndexedDB のパフォーマンスはブラウザの実装とデータ量に依存するが、
一般的なベンチマークでは以下のような傾向が見られる。

- **単一レコードの読み書き**: 0.1-1ms（インデックス使用時）
- **1,000件のバルク挿入**: 10-50ms（単一トランザクション内）
- **10,000件の全件取得**: 50-200ms
- **インデックス検索**: O(log n) のオーダー
- **全件スキャン**: O(n) のオーダー

パフォーマンスを最大化するためのガイドライン:

1. **トランザクションを最小限に**: 複数の操作は単一トランザクションにまとめる
2. **インデックスを適切に設計**: 検索条件に使うプロパティにインデックスを設定する
3. **カーソルより getAll**: 全件取得には `getAll()` が `openCursor()` より高速
4. **読み取り専用トランザクション**: 読み取りのみなら `readonly` を指定する
5. **バージョンアップは最小限に**: 不要なインデックスは作成しない

### Q3: Cookie の SameSite 属性のデフォルト値は何か？

2020 年以降、主要ブラウザは `SameSite` 属性が未指定の Cookie を `SameSite=Lax` として
扱うようになった（Chrome 80 以降、Firefox 69 以降）。

`SameSite=Lax` の挙動:
- 同一サイトからのリクエスト: Cookie を送信する
- クロスサイトの GET リクエスト（リンククリック等）: Cookie を送信する
- クロスサイトの POST リクエスト: Cookie を送信しない
- クロスサイトの iframe: Cookie を送信しない

`SameSite=None` を使う場合は `Secure` 属性が必須である。
これにより、HTTPS 以外の通信でクロスサイト Cookie が送信されることを防いでいる。

### Q4: Web Storage のデータは暗号化されているか？

localStorage と sessionStorage のデータは暗号化されていない。
ブラウザのプロファイルディレクトリにプレーンテキスト（または SQLite）として
保存されている。したがって、ディスクにアクセスできるマルウェアや物理的にデバイスに
アクセスできる攻撃者にはデータが露出する。

機密データをクライアント側に保存する必要がある場合は、
Web Crypto API を使って暗号化してから保存することを検討する。

```javascript
// Web Crypto API による暗号化の例
async function encryptAndStore(key, plaintext, cryptoKey) {
...
async function decryptFromStorage(key, cryptoKey) {
...
### Q5: IndexedDB は SQL をサポートしているか？

IndexedDB は SQL をサポートしていない。IndexedDB はキーバリューストアであり、
NoSQL データベースに分類される。SQL のような柔軟なクエリ（JOIN、GROUP BY、
サブクエリなど）は直接実行できない。

代替手段として以下がある:
1. **インデックス + カーソル**: 基本的なフィルタリングとソート
2. **JavaScript でのフィルタリング**: `getAll()` で取得後に `Array.filter()` / `Array.sort()`
3. **ライブラリの活用**: Dexie.js は SQL ライクなクエリ構文を提供する
4. **OPFS + SQLite (wa-sqlite)**: ブラウザ内で SQLite を WebAssembly で動かす方法
...
- [[01-service-worker-cache.md]] -- Service Worker とキャッシュ戦略の詳細
