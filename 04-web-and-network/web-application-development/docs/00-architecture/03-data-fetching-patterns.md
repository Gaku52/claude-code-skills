...
### Q1: SWRとTanStack Queryの違いは？どちらを選ぶべきか？

**A:** 機能と学習コストのトレードオフで選択します。

**比較表:**

| 観点 | SWR | TanStack Query |
|------|-----|----------------|
| バンドルサイズ | 4KB (gzip) | 11KB (gzip) |
| 学習コスト | 低（シンプルなAPI） | 中（多機能だが複雑） |
| DevTools | なし（非公式あり） | 公式DevTools |
| Mutation | 手動実装 | useMutation で組み込み |
| 無限スクロール | useSWRInfinite（基本的） | useInfiniteQuery（高機能） |
| 楽観的更新 | mutate() で手動 | onMutate で宣言的 |
| リトライ戦略 | 設定可能 | 詳細な設定可能 |
| ページネーション | 手動実装 | 組み込み |
| Query Cancellation | 手動（AbortController） | 自動キャンセル |
| ドキュメント | シンプル | 非常に詳細 |
...
SWR が適している場合:
  ✅ 小〜中規模アプリ（10画面未満）
  ✅ データ読み取り中心（Mutation が少ない）
  ✅ シンプルなキャッシュ戦略で十分
  ✅ バンドルサイズを最小化したい
  ✅ Vercel/Next.js エコシステムを重視

  例: マーケティングサイト、ブログ、小規模SaaS

TanStack Query が適している場合:
  ✅ 中〜大規模アプリ（10画面以上）
  ✅ 複雑なデータ変更フロー（楽観的更新、ロールバック）
  ✅ 無限スクロール、ページネーションが多い
  ✅ DevToolsでデバッグしたい
  ✅ 詳細なキャッシュ制御が必要

  例: 管理画面、データドリブンSaaS、ダッシュボード
```

**実装例の比較:**

```typescript
// SWR: シンプルだがMutationは手動
import useSWR from 'swr';
...
**推奨:** 新規プロジェクトでは TanStack Query を選択し、成長に備える。ただし、シンプルなアプリや学習目的なら SWR で十分。

### Q2: Server Componentsでのデータフェッチングはどう実装するか？

**A:** async/awaitで直接データを取得し、Suspenseで段階的レンダリングを実現します。
...
// 別のServer Componentで独立してデータ取得
async function RecentActivity() {
  // ここのfetchは並列実行される（Waterfall回避）
...
async function UserStats() {
  // Next.js の fetch() はデフォルトでキャッシュされる
...
async function ExternalAPIData() {
  // 低速なAPIはSuspense境界で分離
...
// 4. On-demand Revalidation（タグベース）
...
export async function updateProduct() {
  ...
  revalidateTag('products'); // タグに紐づくキャッシュを無効化
}
```

**ベストプラクティス:**
- 並列データ取得: Promise.all で複数のfetchを同時実行
- エラーハンドリング: error.tsx で境界を設定
- ローディング状態: loading.tsx または Suspense で段階的レンダリング

### Q3: Waterfall問題とその解決方法は？

**A:** データ取得が直列化（連鎖的）になり、パフォーマンスが悪化する問題です。

**Waterfall問題の例:**

```typescript
// ❌ BAD: 逐次的なデータ取得（Waterfall）
function ProductPage({ id }: { id: string }) {
  ...
  // product取得完了後にreviewsを取得開始
  const { data: reviews } = useQuery({
    queryKey: ['reviews', id],
    queryFn: () => fetchReviews(id),
    enabled: !!product, // productがないと実行されない
  });

  // reviews取得完了後にrecommendationsを取得開始
  ...
  // 合計時間 = product(500ms) + reviews(300ms) + recommendations(400ms) = 1200ms
}
```

**解決策1: 並列化（useQueries / Promise.all）**
...
function ProductPage({ id }: { id: string }) {
  const [productQuery, reviewsQuery, recommendationsQuery] = useQueries({
    queries: [
      { queryKey: ['product', id], queryFn: () => fetchProduct(id) },
      { queryKey: ['reviews', id], queryFn: () => fetchReviews(id) },
      { queryKey: ['recommendations', id], queryFn: () => fetchRecommendations(id) },
    ],
  });

  // 合計時間 = max(500ms, 300ms, 400ms) = 500ms（67%改善）

  const isLoading = productQuery.isLoading || reviewsQuery.isLoading || recommendationsQuery.isLoading;
  ...
  return (
    <>
      ...
      <Recommendations items={recommendationsQuery.data} />
    </>
  );
}
```

**解決策2: Suspenseで段階的レンダリング（Server Components）**

```typescript
// ✅ GOOD: Render-As-You-Fetch パターン
...
**結論:**
- クライアント: useQueries で並列化
- サーバー: Suspense境界を活用してStreaming SSR
- ナビゲーション: Prefetchで体感速度を向上

---

## 次に読むべきガイド

→ [[../01-state-management/00-overview.md]] -- 状態管理概論
→ [[00-spa-mpa-ssr.md]] -- レンダリングパターン（SSR/SSG/ISR）
