...
- **クライアントサイドルーティング**: ルーティングの基本概念と History API の仕組み → `./00-client-side-routing.md`
...
これらの概念を理解することで、ファイルベースルーティングの設計パターンを効果的に適用できる。
...
**A:** 段階的移行が推奨される。`app/` と `pages/` は共存可能なため、新規ページから App Router で実装し、既存ページは必要に応じて移行する。

```
移行戦略:

Phase 1: 共存期間の開始
  ├── pages/          # 既存のPages Router（そのまま稼働）
  │   ├── index.tsx
  │   ├── about.tsx
  │   └── users/
  │       └── [id].tsx
  └── app/            # 新規ページはApp Routerで実装
      ├── layout.tsx   # ルートレイアウト
      └── dashboard/   # 新規機能
          └── page.tsx
...
注意点:
  - 同じパスで pages/ と app/ が競合する場合、app/ が優先される
  - `_app.tsx` のロジックは app/layout.tsx に移行
  - `getServerSideProps` → Server Component or Route Handler
  - `getStaticProps` → generateStaticParams
```

### Q2: 動的ルートと catch-all ルートの使い分けは?

**A:** URL 構造が事前に定義されている場合は動的ルート `[id]`、可変長のパスや柔軟なルーティングが必要な場合は catch-all `[...slug]` を使用する。
...
// =========================================
// URL: /blog または /blog/2024/01/hello
...
export default async function BlogPage({
  ...
  // /blog/2024/01/hello → 記事詳細
  return <BlogPost slug={params.slug} />;
}

使い分けの基準:
  [id]         → 単一パラメータ（/users/123, /posts/abc）
  [...slug]    → 可変長パス（/docs/a/b/c）、必須
  [[...slug]]  → 可変長パス、省略可能（/blog でもマッチ）
```

### Q3: ルートグループの活用法は?

**A:** ルートグループ `(name)` は URL に影響を与えず、レイアウトやミドルウェアのスコープを分けるために使用する。
...
└── [locale]/
    ├── (shop)/            # ECサイト
    │   ├── layout.tsx
    │   └── products/page.tsx  # → /en/products, /ja/products
    └── (blog)/            # ブログ
        ├── layout.tsx
        └── posts/page.tsx     # → /en/posts, /ja/posts

4. A/Bテスト
app/
├── (variant-a)/
│   └── pricing/page.tsx   # → /pricing（バリアントA）
└── (variant-b)/
    └── pricing/page.tsx   # → /pricing（バリアントB）
# middleware.ts でルーティング制御
